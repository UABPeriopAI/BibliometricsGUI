...
def build_coauthor_network(df):
    ...
    for authors in df['author_names']:
        # Split author names and clean up whitespace
        author_list = [author.strip() for author in authors.split(';')]
        # Generate all co-author pairs
...
if data_source == "Upload Spreadsheet":
    ...
    def format_scopus_query(author, start_year, end_year, afid, institution):
        ...
        if institution:
            # Support multiple levels of institutions (e.g., department and school) connected by AND
    ...
    if st.sidebar.button("Execute Query"):
        if not scopus_query:
            scopus_query = format_scopus_query(author, start_year, end_year, city, journal, institution)
...
# Main App: Display, Enrichment, and Visualization
    ...
    if not df.empty:
        ...
        if st.button("Launch PyGWalker"):
            pyg.walk(df)  # This will open PyGWalker in a new browser window
        
        if not df.empty and "author_names" in df.columns:
            st.write("### Co-Author Network Visualization")
        
            # Build the network
            coauthor_network = build_coauthor_network(df)
        
            # Function to filter the graph based on a threshold
            ...
            pos = nx.spring_layout(filtered_coauthor_network, seed=42, k=0.7)  # Adjust layout for better readability
            node_sizes = [100 + (degree * 10) for _, degree in filtered_coauthor_network.degree()]
            nx.draw_networkx_nodes(filtered_coauthor_network, pos, node_size=node_sizes, node_color="lightblue")
            ...
            # Display the visualization in the Streamlit app
            st.pyplot(fig)
        else:
