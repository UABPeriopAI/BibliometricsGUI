...
if not df.empty:
    ...
    year_options = monthly_counts['Year'].unique()
    selected_years = st.multiselect('Select Years to Display', year_options, default=year_options)
    filtered_monthly_counts = monthly_counts[monthly_counts['Year'].isin(selected_years)]
    filtered_monthly_counts['YearMonth'] = filtered_monthly_counts.apply(
    ...
    fig1 = px.line(
        ...
        y='Count', 
        markers=True, 
        title="Filtered Monthly Publication Trend",
        labels={'YearMonth': 'Time (Year-Month)', 'Count': 'Publication Count'}
    )
