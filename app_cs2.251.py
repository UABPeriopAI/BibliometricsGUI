...
def fetch_data_for_dois(dois):
    """
    Fetch publication data from Scopus for a list of DOIs.
    """
    publication_data = []
    for doi in dois:
        clean_doi = doi.strip()  # Remove any leading/trailing spaces
        st.info(f"Processing DOI: {clean_doi}")  # Display the DOI being processed
        query = f'DOI("{clean_doi}")'  # Wrap the DOI in quotes for a strict Scopus query
        scopus_data = fetch_scopus_data(query)  # Use the existing Scopus query function
        if not scopus_data.empty:
            publication_data.append(scopus_data.iloc[0])  # Assume one result per DOI
        else:
            st.warning(f"No result found for DOI: {clean_doi}")  # Warn if no result is found
    if publication_data:
