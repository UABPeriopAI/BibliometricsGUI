...
if not df.empty:
    ...
    st.write(df.head(20))  # Adjusted to display the top 20 rows
    ...
    if aggregation_option == "Monthly":
        ...
        st.subheader("Publication Counts Monthly")
    ...
    if aggregation_option == "Monthly":
        ...
        if graph_type == "Line":
            fig = px.line(complete_df, x=x_axis, y='Count', markers=True, title=title)
            fig.update_yaxes(dtick=1)
        elif graph_type == "Bar":
            fig = px.bar(complete_df, x=x_axis, y='Count', title=title)
            fig.update_yaxes(dtick=1)
        elif graph_type == "Scatter":
            fig = px.scatter(complete_df, x=x_axis, y='Count', title=title)
            fig.update_yaxes(dtick=1)
    elif aggregation_option == "Quarterly":
        x_axis = 'YearQuarter'
